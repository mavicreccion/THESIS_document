...
\noindent where $r_s$  represents the Spearman’s rank correlation coefficient, $a$ denotes the ranked difference between the $i$th measurements for the two random variables, and $N$ is the number of measurements in each of the two random variables in the correlation \shortcite{Zar1972SignificanceCoefficient}. $r_s$ measures the strength and course of the \textit{monotonic} relationship between ranked variables. \shortciteA{Hauke2011ComparisonData}, meanwhile, explain that $r_s$ can be considered as the correlation coefficient of Pearson that deals with ranks.  
...
\noindent where $k$ denotes the lags, $N$ as the number of observations in the series, and $X$ as the data.
